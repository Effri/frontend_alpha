...
function enableChatInput(enable) {
    ...
    if (enable != (chat.input != null)) {
        ...
        if (enable) {
            ...
            chat.input = chatvar.append('<div><input value="" id="chat_msg" type="text"/><div class="chat-btns"><button onclick="doInChat(writeDo)">/do</button><button onclick="doInChat(writeMe)">/me</button><button onclick="doInChat(writeTry)">/try</button><button onclick="doInChat(writeB)">/b</button></div></div></ul>').children(":last");
